...
function getBackendPath() {
    if (isDev) {
        ...
        return {
            command: backendPath,
            args: [],
            cwd: path.dirname(backendPath) || process.resourcesPath
        };
    }
}
...
function startBackend() {
    ...
    if (process.platform === 'win32') {
        try {
            const findCmd = `netstat -ano | findstr :${API_PORT}`;
            const stdout = require('child_process').execSync(findCmd).toString();
            const lines = stdout.trim().split('\n');
            for (const line of lines) {
                const parts = line.trim().split(/\s+/);
                const pid = parts[parts.length - 1];
                if (pid && !isNaN(pid) && pid !== '0') {
                    log(`Killing existing process on port ${API_PORT} (PID: ${pid})`);
    ...
    <button onclick="location.reload()">Retry</button>
        ...
        if (isDev) {
            ...
            if (fs.existsSync(frontendPath)) {
                ...
                const altFrontendPath = path.join(__dirname, '..', 'frontend', 'dist', 'index.html');
                log(`Trying alternative path: ${altFrontendPath}`);
                log(`Alternative path exists: ${fs.existsSync(altFrontendPath)}`);
                
                if (fs.existsSync(altFrontendPath)) {
                    mainWindow.loadFile(altFrontendPath).catch(err => {
                        log(`Failed to load frontend from alternative path: ${err.message}`);
                        createWindow(`Failed to load frontend: ${err.message}`);
                    });
                } else {
          ...
          <html>
            ...
            <body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; padding: 40px; background: #f5f5f5;">
              ...
              <p>Alternative path: ${altFrontendPath}</p>
