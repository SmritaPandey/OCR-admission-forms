...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      // Load recent forms only (10 items)
      const recentForms = await apiService.listForms(0, 10);
      setForms(recentForms);
      
      // Use smaller batches for stats to avoid timeout
      // Get stats from smaller sample and estimate
      const sampleForms = await apiService.listForms(0, 100);
      
      let documentCount = 0;
      try {
        // Use smaller limit for documents
        const documents = await apiService.searchDocuments({ limit: 100 });
        documentCount = documents.length;
        // Estimate total if we got full batch
        if (documents.length === 100) {
          documentCount = 100; // Just show "100+" or estimate
        }
      } catch (err) {
      ...
      let studentCount = 0;
      try {
        // Use smaller limit for students
        const students = await apiService.listStudentProfiles(0, 100);
        studentCount = students.length;
        // Estimate total if we got full batch
        if (students.length === 100) {
          studentCount = 100; // Just show "100+" or estimate
        }
      } catch (err) {
        console.error('Failed to load students:', err);
      }
      
      // Calculate stats from sample and estimate
      const verifiedCount = sampleForms.filter(f => f.status === 'verified').length;
      const pendingCount = sampleForms.filter(f => f.status === 'extracted' || f.status === 'uploaded').length;
      
      setStats({
        total: sampleForms.length >= 100 ? 100 : sampleForms.length, // Show sample size or actual
  ...
  }, []); // Only run on mount, loadDashboardData is stable
