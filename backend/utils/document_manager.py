...
class DocumentManager:
    ...
    def _ensure_directories(self):
        ...
        try:
            ...
            # If we can't create directories, log error but don't crash
            import sys
            print(f"WARNING: Could not create directories in {self.documents_dir}: {e}", file=sys.stderr)
            print(f"UPLOAD_DIR setting: {settings.UPLOAD_DIR}", file=sys.stderr)
            # Try to use a fallback location
            import tempfile
            fallback_dir = Path(tempfile.gettempdir()) / "ocr_form_extractor" / "uploads"
            fallback_dir.mkdir(parents=True, exist_ok=True)
            self.upload_dir = fallback_dir
            self.documents_dir = fallback_dir / "documents"
            self.documents_dir.mkdir(parents=True, exist_ok=True)
            (self.documents_dir / "by_student").mkdir(exist_ok=True)
            (self.documents_dir / "by_category").mkdir(exist_ok=True)
