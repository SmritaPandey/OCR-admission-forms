...
class GoogleOCREnhancer:
    ...
    def _is_label_text(self, value: str) -> bool:
        ...
        if HAVE_FORM_LABELS:
            if is_form_label(value_lower, strict=True):
                return True
        ...
        label_patterns = [
            ...
            # Additional patterns
            r'name\s+in\s+block\s+letters', r'block\s+letters',
            r'first\s+name', r'middle\s+name', r'surname', r'last\s+name',
            r'date\s+of\s+birth', r'date\s+of\s+admission',
            r'permanent\s+address', r'correspondence\s+address',
            r'^state\s*$', r'^pin\s*$', r'^pincode\s*$',
            r'contact\s+numbers?', r'mobile\s+no', r'phone\s+no',
            r"mother'?s?\s+name", r"father'?s?\s+name", r"guardian'?s?\s+name",
            r'occupation', r'designation', r'organization',
            r'^male\s*$', r'^female\s*$', r'^transgender\s*$',
            r'^yes\s*$', r'^no\s*$',
        ]
        ...
        if value.isupper() and len(value.split()) <= 3 and len(value) < 30:
            # But allow course codes and categories
            if value not in ['B.COM.(H)', 'B.A.(H) ECO', 'GEN', 'OBC', 'SC', 'ST', 'EWS', 'PWD']:
                return True
