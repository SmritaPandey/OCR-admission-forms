...
function StudentSearch() {
  ...
  const loadStudents = useCallback(async () => {
    try {
      ...
      // Always filter for verified students only
      const filters = Object.keys(advancedFilters).length > 0 
        ? { ...advancedFilters, is_verified: true }
        : { is_verified: true };
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      ...
      const exportParams: Record<string, string | boolean> = { is_verified: true }; // Only export verified students
